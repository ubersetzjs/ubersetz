import type { AutotranslationFunction } from '@ubersetz/cli/dist/types'
...
function simplifyTargetLang(langCode: TargetLanguage): TargetLanguageCode {
  if (langCode === 'pt-PT') return langCode
...
const translate: AutotranslationFunction = async (options) => {
  ...
  const sourceLanguage = options.sourceLanguage
    ? options.sourceLanguage.slice(0, 2).toLowerCase() as SourceLanguage
    : undefined
  ...
  const targetLanguage = `${options.targetLanguage.slice(0, 2).toLowerCase()}-${options.targetLanguage.slice(3, 5).toUpperCase()}` as TargetLanguage
  ...
  if (process.env.DEEPL_API_KEY) {
    ...
    return {
      text: preserveVariables(options.text, textResult.text as string),
    }
  }
